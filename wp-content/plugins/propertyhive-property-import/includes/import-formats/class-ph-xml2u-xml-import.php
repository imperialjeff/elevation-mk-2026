<?php
/**
 * Class for managing the import process of an XML2U file
 *
 * @package WordPress
 */
if ( class_exists( 'PH_Property_Import_Process' ) ) {

class PH_Xml2u_XML_Import extends PH_Property_Import_Process {
    ...
    public function import()
    {
        ...
        $geocoding_denied = apply_filters( 'propertyhive_xml2u_xml_import_prevent_geocoding', false, $this->import_id );

        do_action( "propertyhive_pre_import_properties_xml2u_xml", $this->properties, $this->import_id );
        $this->properties = apply_filters( "propertyhive_xml2u_xml_properties_due_import", $this->properties, $this->import_id );
        ...
        foreach ( $this->properties as $property )
        {
            ...
            do_action( "propertyhive_property_importing_xml2u_xml", $property, $this->import_id, $this->instance_id );
            ...
            $display_address = (string)$property->title;
            if ( $display_address == '' )
            {
                $display_address = (string)$property->location;
                if ( (string)$property->subRegion != '' )
                {
                    $display_address = ', ' . (string)$property->subRegion;
                }
            }
            if ( $display_address == '' )
            {
            ...
            if ( $display_address == '' )
            {
                $display_address = (string)$property->region;
            }

            list( $inserted_updated, $post_id ) = $this->insert_update_property_post( (string)$property->propertyid, $property, $display_address, (string)$property->shortDescription );
            ...
            if ( $inserted_updated !== false )
            {
                ...
                $previous_update_date = get_post_meta( $post_id, '_xml2u_update_date_' . $this->import_id, TRUE);
                ...
                if ( isset($import_settings['only_updated']) && $import_settings['only_updated'] == 'yes' )
                {
                    if (
                        ...
                        !isset($property->lastUpdateDate) ||
                        (
                            isset($property->lastUpdateDate) &&
                            trim((string)$property->lastUpdateDate) == ''
                        ) ||
                        $previous_update_date == '' ||
                        (
                            isset($property->lastUpdateDate) &&
                            (string)$property->lastUpdateDate != '' &&
                            $previous_update_date != '' &&
                            strtotime((string)$property->lastUpdateDate) > strtotime($previous_update_date)
                        )
                    )
                    {
                        $skip_property = false;
                ...
                {
                    // Address
                    update_post_meta( $post_id, '_reference_number', (string)$property->Price->reference );
                    update_post_meta( $post_id, '_address_name_number', ( ( isset($property->Address->number) ) ? (string)$property->Address->town : '' ) );
                    update_post_meta( $post_id, '_address_street', ( ( isset($property->Address->street) ) ? (string)$property->Address->street : '' ) );
                    update_post_meta( $post_id, '_address_two', ( ( isset($property->Address->location) ) ? (string)$property->Address->location : '' ) );
                    update_post_meta( $post_id, '_address_three', ( ( isset($property->Address->subRegion) ) ? (string)$property->Address->subRegion : '' ) );
                    ...
                    $country = 'ES';
                    if ( isset($property->Address->{'countryCodeISO3166-1-alpha2'}) && (string)$property->Address->{'countryCodeISO3166-1-alpha2'} != '' )
                    {
                    ...
                    $currency = 'EUR';
                    if ( isset($property->Price->currency) && (string)$property->Price->currency != '' )
                    {
                        $currency = (string)$property->Price->currency;
                    }

                    // Let's just look at address fields to see if we find a match
                    $address_fields_to_check = apply_filters( 'propertyhive_xml2u_xml_address_fields_to_check', array('location', 'subRegion', 'region'), $this->import_id );
                    ...
                    else
                    {
                        ...
                        if ( !$geocoding_denied && ( $lat == '' || $lng == '' || $lat == '0' || $lng == '0' ) )
                        {
                            ...
                            if ( isset($property->Address->location) && trim((string)$property->Address->location) != '' ) { $address_to_geocode[] = (string)$property->Address->location; }
                            if ( isset($property->Address->subRegion) && trim((string)$property->Address->subRegion) != '' ) { $address_to_geocode[] = (string)$property->Address->subRegion; }
                            if ( isset($property->Address->region) && trim((string)$property->Address->region) != '' ) { $address_to_geocode[] = (string)$property->Address->region; }

                            $geocoding_return = $this->do_geocoding_lookup( $post_id, (string)$property->propertyid, $address_to_geocode, $address_to_geocode, $country );
                    ...
                    if ( isset($property->category) && strpos(strtolower((string)$property->category), 'for sale') === false )
                    {
                    ...
                    if ( 
                        ...
                        strpos(strtolower((string)$property->category), 'commercial') !== false 
                    )
                    {
                    ...
                    update_post_meta( $post_id, '_bedrooms', ( ( isset($property->Description->beds) ) ? (string)$property->Description->beds : '' ) );
                    update_post_meta( $post_id, '_bathrooms', ( ( isset($property->Description->fullBathrooms) ) ? (string)$property->Description->fullBathrooms : '' ) );
                    update_post_meta( $post_id, '_reception_rooms', ( ( isset($property->Description->receptionRooms) ) ? (string)$property->Description->receptionRooms : '' ) );
                    ...
                    {
                        $property_type = (string)$property->Description->propertyType;
                        ...
                        else
                        {
                            ...
                            $this->log( 'Property received with a type (' . $property_type . ') that is not mapped', $post_id, (string)$property->propertyid );
                    ...
                    elseif ( $department == 'residential-lettings' )
                    {
                        ...
                        update_post_meta( $post_id, '_deposit', ( ( isset($property->Price->RentalBond) ) ? (string)$property->Price->RentalBond : '' ) );
                        update_post_meta( $post_id, '_available_date', ( ( isset($property->Price->availableDate) ) ? (string)$property->Price->availableDate : '' ) );
                    }
                    elseif ( $department == 'commercial' )
                    {
                        ...
                        if ( isset($property->category) && strpos(strtolower((string)$property->category), 'for sale') === false )
                        {
                            ...
                            if ( isset($property->Price->price) && is_numeric((string)$property->Price->price) )
                            {
                                $rent = round(preg_replace("/[^0-9.]/", '', (string)$property->Price->price));
                            }
                        ...
                        $size = preg_replace("/[^0-9.]/", '', (string)$property->Description->FloorSize->floorSize);
                        $units = 'sqft';
                        if ( strpos(strtolower((string)$property->Description->FloorSize->floorSizeUnits), 'met') )
                        {
                            $units = 'sqm';
                        }
                        elseif ( strpos(strtolower((string)$property->Description->FloorSize->floorSizeUnits), 'acr') )
                        {
                            $units = 'acre';
                        }
                        elseif ( strpos(strtolower((string)$property->Description->FloorSize->floorSizeUnits), 'hect') )
                        {
                            $units = 'hectare';
                        }
                        ...
                        $size = preg_replace("/[^0-9.]/", '', (string)$property->Description->PlotSize->plotSize);
                        $units = 'sqft';
                        if ( strpos(strtolower((string)$property->Description->PlotSize->plotSizeUnits), 'met') )
                        {
                            $units = 'sqm';
                        }
                        elseif ( strpos(strtolower((string)$property->Description->PlotSize->plotSizeUnits), 'acr') )
                        {
                            $units = 'acre';
                        }
                        elseif ( strpos(strtolower((string)$property->Description->PlotSize->plotSizeUnits), 'hect') )
                        {
                            $units = 'hectare';
                        }
                        ...
                        update_post_meta( $post_id, '_site_area_from_sqft', convert_size_to_sqft( $size, $units ) );

                        update_post_meta( $post_id, '_site_area_to', $size );

                        update_post_meta( $post_id, '_site_area_to_sqft', convert_size_to_sqft( $size, $units ) );
                    ...
                    for ( $i = 1; $i <= 10; ++$i )
                    {
                        ...
                        {
                            $features[] = trim((string)$property->Features->{'Feature' . $i});
                        }
                    }
                    ...
                    if ( isset($property->Description->description->en) && (string)$property->Description->description->en != '' )
                    {
                        update_post_meta( $post_id, '_rooms', '1' );
                        update_post_meta( $post_id, '_room_name_0', '' );
                        update_post_meta( $post_id, '_room_dimensions_0', '' );
                        update_post_meta( $post_id, '_room_description_0', (string)$property->Description->description->en );
                    }
                        ...
                        foreach ($property->images as $images)
                        {
                            if (!empty($images->image))
                            {
                                foreach ($images->image as $image)
                                {
                                    ...
                                    $modified = (string)$property->lastUpdateDate . ' 00:00:00';

                                    $media[] = array(
                                        'url' => $url,
                                        'filename' => basename( $url ) . '.jpg',
                                        'description' => isset($image->alttext) ? (string)$image->alttext : '',
                                        'modified' => $modified,
                                    );
                                }
                            }
                ...
                $update_date = '';
                if ( isset($property->lastUpdateDate) && (string)$property->lastUpdateDate != '' )
                {
                    $update_date = date("Y-m-d H:i:s", strtotime((string)$property->lastUpdateDate));
                }
                update_post_meta( $post_id, '_xml2u_update_date_' . $this->import_id, $update_date );

                do_action( "propertyhive_property_imported", $post_id, $property, $this->import_id );
                do_action( "propertyhive_property_imported_xml2u_xml", $post_id, $property, $this->import_id );
        ...
        do_action( "propertyhive_post_import_properties_xml2u_xml", $this->import_id );
