...
final class PH_ValPal {
    ...
    public function valpal_shortcode() 
    {
        ...
        $translation_array = apply_filters( 'propertyhive_valpal_translation_array', array(
            ...
            'maps_provider' => get_option( 'propertyhive_maps_provider' ),
            'show_map_in_results' => apply_filters( 'propertyhive_valpal_show_map_in_results', true ),
            'show_street_view_in_results' => get_option( 'propertyhive_maps_provider' ) == '' ? apply_filters( 'propertyhive_valpal_show_street_view_in_results', true ) : false,
        ) );
    ...
    {
        $template = locate_template( array('propertyhive/valpal-results.php') );
        if ( !$template )
        {
            include( dirname( PH_VALPAL_PLUGIN_FILE ) . '/templates/valpal-results.php' );
        }
        else
        {
            include( $template );
    ...
    public function do_postcode_lookup()
    {
        ...
        $contents = file_get_contents('https://www.valpal.co.uk/addressfinder/stepone.php?postcode=' . urlencode($postcode) . '&username=' . urlencode($current_settings['address_lookup_username']) . '&password=' . urlencode($current_settings['address_lookup_password']));
    ...
    public function do_val_request()
    {
        ...
        if ( isset($current_settings['address_lookup']) && $current_settings['address_lookup'] == '1' ) 
        {
            ...
            $contents = file_get_contents('https://www.valpal.co.uk/addressfinder/steptwo.php?lstResults=' . $property . '&username=' . urlencode($current_settings['address_lookup_username']) . '&password=' . urlencode($current_settings['address_lookup_password']));
        ...
        else
        {
            ...
            if ( isset($body[0]['status']) && $body[0]['status'] == 'success' )
            {
                ...
                $lng = '';
                $geocoding_error = '';
                $address_to_geocode = $number . ' ' . $street . ', ' . $postcode;
                if ( get_option('propertyhive_geocoding_provider') == 'osm' )
                {
                    $request_url = "https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=gb&addressdetails=1&q=" . urlencode($address_to_geocode);
                    ...
                    if ( !is_wp_error( $response ))
                    {
                        if ( is_array( $response ) )
                        {
                            ...
                            {
                                $geocoding_error = print_r($json, true);
                            }
                        }
                        else
                        {
                            $geocoding_error = print_r($response, true);
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if ( $api_key != '' )
                    {
                        $request_url = "https://maps.googleapis.com/maps/api/geocode/xml?address=" . urlencode( $address_to_geocode ) . "&sensor=false&region=gb";
                        ...
                        if ( is_array( $response ) && !is_wp_error( $response ) ) 
                        {
                            ...
                            if ( $xml !== FALSE )
                            {
                                ...
                                {
                                    $geocoding_error = 'Status: ' . $status;
                                }
                            }
                            else
                            {
                                $geocoding_error = print_r($xml, true);
                            }
                        }
                        else
                        {
    ...
    public function save() {
        ...
        $propertyhive_valpal = array(
            'username' => ( (isset($_POST['username'])) ? ph_clean(wp_unslash($_POST['username'])) : '' ),
            'password' => ( (isset($_POST['password'])) ? ph_clean(wp_unslash($_POST['password'])) : '' ),
            ...
            'address_lookup_username' => ( (isset($_POST['address_lookup_username'])) ? ph_clean(wp_unslash($_POST['address_lookup_username'])) : '' ),
            'address_lookup_password' => ( (isset($_POST['address_lookup_password'])) ? ph_clean(wp_unslash($_POST['address_lookup_password'])) : '' ),
            'valuation_form_disclaimer' => ( (isset($_POST['valuation_form_disclaimer'])) ? wp_kses_post( trim( wp_unslash($_POST['valuation_form_disclaimer'] ) ) ) : '' ),
        );
