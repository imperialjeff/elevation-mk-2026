...
function propertyhive_property_import_get_all_node_names( $node, $names, $parents = array() )
{
    $children = false;
    $nodes_names = array();
    ...
    foreach ( $names as $name )
    {
        ...
        if ( count($node->children($name)) ) 
        {
            ...
            {
                $new_node_names = propertyhive_property_import_get_all_node_names($child, $names, $new_parents);
...
function propertyhive_property_import_simplexml_to_array_with_cdata_support( $xml )
{   
    $array = (array)$xml;
    ...
    foreach ( $array as $key => $value ) 
    {
        ...
        $array[$key] = propertyhive_property_import_simplexml_to_array_with_cdata_support($value);
    }
